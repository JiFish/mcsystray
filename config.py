...
class config():
    ...
    def load(self, file='config.ini'):
        ...
        self.corsairkeyname = self.get(
            'CorsairKeyName', section='corsair', default='PauseBreak')
        self.keycol[STATUS_DISABLED] = self.gethex(
            'DisabledColor', section='corsair', default=(128, 128, 128))
        self.keycol[STATUS_OFFLINE] = self.gethex(
            'OfflineColor', section='corsair', default=(255, 0, 0))
        self.keycol[STATUS_ONLINE] = self.gethex(
            'OnlineColor', section='corsair', default=(0, 255, 0))
        self.keycol[STATUS_INUSE] = self.gethex(
        ...
        self.trayicon[STATUS_DISABLED] = self.get(
            'DisabledIcon', section='icons', default='icon_grey.png')
        self.trayicon[STATUS_OFFLINE] = self.get(
            'OfflineIcon', section='icons', default='icon_red.png')
        self.trayicon[STATUS_ONLINE] = self.get(
            'OnlineIcon', section='icons', default='icon_green.png')
        self.trayicon[STATUS_INUSE] = self.get(
            'InUseIcon', section='icons', default='icon_blue.png')
